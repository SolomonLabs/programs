import { AmmClient, AUTOCRAT_PROGRAM_ID, AutocratClient, ConditionalVaultClient, getProposalAddr, InstructionUtils } from "@metadaoproject/futarchy/v0.5";
import { LAMPORTS_PER_SOL, Message, PublicKey, SystemProgram, Transaction, TransactionMessage, VersionedTransaction } from "@solana/web3.js";
...
import { convertTransactionInstruction, convertCompiledInstruction, getTipAccounts, sendBundle, getTipFloor, getBundleStatuses } from "../utils/bundles.js";
...
async function main() {
  ...
  const payerBaseTokenAccount = getAssociatedTokenAddressSync(dao.baseMint, payer.publicKey);
  const payerQuoteTokenAccount = getAssociatedTokenAddressSync(dao.quoteMint, payer.publicKey);

  try {
    const baseTokenAccount = await provider.connection.getTokenAccountBalance(payerBaseTokenAccount);
    console.log("baseTokenAccount", baseTokenAccount.value);
    assert(Number(baseTokenAccount.value.amount.toString()) >= Number(minBaseLiquidity.toString()), "Base token account balance does not match minBaseLiquidity");
  } catch (error) {
    console.log(error);
  ...
  try {
    const quoteTokenAccount = await provider.connection.getTokenAccountBalance(payerQuoteTokenAccount);
    console.log("quoteTokenAccount", quoteTokenAccount.value);
    assert(Number(quoteTokenAccount.value.amount.toString()) >= Number(minQuoteLiquidity.toString()), "You don't have enough quote tokens to create a market");
    assert(Number(quoteTokenAccount.value.amount.toString()) >= Number(100_000_001), "Cannot create market with less than 100 USDC");
  } catch (error) {
    console.log(error);
    console.log("quoteTokenAccount not found");
  }
  
  minQuoteLiquidity = Number(minQuoteLiquidity.toString()) < Number(100_000_001) ? new BN(100_000_001) : minQuoteLiquidity;
  ...
  const proposalTx = await autocratClient.initializeProposalIx(
    "", proposalKey, DAO_KEY, dao.baseMint, dao.quoteMint, lpTokens, lpTokens, question
  ).transaction();
...
const prepareBundle = async (transactions: Transaction[]) => {
  ...
  try {
    ...
    const signedTxns = await provider.wallet.signAllTransactions([...transactions]);
